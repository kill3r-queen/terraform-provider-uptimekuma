...
func TestMonitorOperations(t *testing.T) {
	...
	// Using value type here as the handler logic primarily reads or replaces whole elements
	monitors := []Monitor{
		{
			ID:            1,
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path == "/monitors" {
			...
			parts := strings.Split(trimmedPath, "/") // e.g., ["monitors", "1", "pause"]
			...
			var monitorIndex = -1
			for i := range monitors { // Iterate by index to modify/delete later
				if monitors[i].ID == id {
					monitorIndex = i
					break
				}
			...
			if action != "" {
				...
				case "beats":
					...
					return
				case "tag":
					// --- Start Fix for staticcheck QF1003 ---
					switch r.Method {
					case http.MethodPost:
						// Add tag logic (simplified)
						var tagData map[string]interface{}
						// Note: Ensure decoding the body is the correct expectation for AddTag
					...
					// --- End Fix for staticcheck QF1003 ---
				default:
	...
	if err == nil {
		...
		// This depends on how your client wraps/returns errors
